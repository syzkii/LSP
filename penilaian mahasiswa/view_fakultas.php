<?
	...
	if ($_POST["simpan"])
	{
		...
		$q_simpan="insert into fakultas (fakultas) values ('".$fakultas."')";
  ...
  <table cellpadding='0' cellspacing='0' width='80%' align='center'>
	<tr>
		<td width='100%'>
			<br><br>
			<table width='100%'>
			<form method='post' action='view_fakultas.php'>
			<tr>
				<td>Cari Data
				<br><br>
				...
				<td width='50%' align='left'>
					...
					<a href='add_fakultas.php' class='btn btn-dark btn-kere2'><i class="fa fa-address-book-o">Tambah Data</a></i>
				</td>
			</tr>
			</form>
			</table>
			...
			if($_POST["cari"])
			{
				$sql_cari = "where (id like '%".$_POST["keyword"]."%' or fakultas like '%".$_POST["keyword"]."%') ";
			}
			$result = mysql_query("SELECT * FROM fakultas");

			$show = " select * from fakultas ".$sql_cari." ORDER BY id ";
			...
			while ($data = mysql_fetch_assoc($query))
				...
				<TR>
					...
					<TD><center>
						<a href='edit_fakultas.php?id=<?php echo $data["id"]; ?>' class="btn btn-warning"><i class="fa fa-edit">Edit</a></i>
						<a href='delete_fakultas.php?id=<?php echo $data["id"]; ?>' class="btn btn-danger"><i class="fa fa-eraser">Delete</a></i>
					</TD>
				</TR>				
				<?
					}
